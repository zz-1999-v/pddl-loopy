...
# DRAKE_IIWA_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/ur5_hand.urdf')
...
TABLE_URDF = join_paths('/home/zz/lupy/urdf/objects/table.urdf')
...
# UR5 Robots
# UR5_URDF = join_paths('/home/zz/pddlstream/examples/pybullet/loopy/sim-master/urdfs/ur5.urdf')
UR5_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/sim-master/urdfs/ur5.urdf')
# UR5_ARM_URDF = join_paths('/home/zz/lupy/urdf/urdf/ur5_hand.urdf') 
# UR5_UNI_ARM_URDF = join_paths('/home/zz/pddlstream/examples/pybullet/loopy/univer-sim/urdf/ur5_hand.urdf') 
CAMERA_SLOT_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/cup.urdf') 
# GAIZI_URDF = join_paths('/home/zz/pddlstream/examples/pybullet/loopy/sim-master/urdfs/gaizi.urdf')
# GAIZI_URDF = join_paths('/home/zz/pddlstream/examples/pybullet/loopy/univer-sim/urdf/gaizi.urdf')
GAIZI_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/gaizi.urdf') 
TAIZI_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/taizi.urdf')
DRILL_URDF = join_paths(MODEL_DIRECTORY, 'loopy_ur5/035_power_drill/ele_tool.urdf') 
# DRILL_URDF = join_paths('/home/zz/lupy/YCB_Video_Models/models/035_power_drill/ele_tool.urdf')
#####################################
...
def get_link_pose(body, link):
    ...
    # print("*********get_lnik_state***************************************")
    # print("link_state:",link_state.worldLinkFramePosition, link_state.worldLinkFrameOrientation)
...
def get_collision_data(body, link=BASE_LINK):
    # TODO: try catch
    # TODO: cache
    # print("CollisionShapeData", body, link, p.getCollisionShapeData(body, link, physicsClientId=CLIENT))
...
def vertices_from_data(data):
    geometry_type = get_data_type(data)
    print("*******get_data_type************************")
    print('data_type', get_data_type(data))
    #if geometry_type == p.GEOM_SPHERE:
    #    parameters = [get_data_radius(data)]
    if geometry_type == p.GEOM_BOX:
...
def vertices_from_link(body, link=BASE_LINK, collision=True):
    ...
    for data in get_data(body, link):
        ...
        print("*************vertices***********************")
        print(vertices, data)
        print('get_data_pose', get_data_pose(data))
        print('vertices_from_data', vertices_from_data(data))
...
def vertices_from_rigid(body, link=BASE_LINK):
    ...
    try:
        # print("*************link_name****************************")
        # num_links = p.getNumJoints(body)
        # # Print link names and their IDs
        # for link_id in range(num_links):
        #     link_info = p.getJointInfo(body, link_id)
        #     link_name = link_info[1].decode('utf-8')  # Convert bytes to string
        #     print(f"Link ID: {link_id}, Link Name: {link_name}")
        # body_info = p.getBodyInfo(body)
        # robot_name = body_info[1].decode('utf-8')  # 名称是 body_info 的第二个元素
        # # print(f"Robot Name: {robot_name}")
        # if (robot_name == "drill_ele") : link = 4
        ...
        if ext == '.obj':
            if info.path not in OBJ_MESH_CACHE:
                print("***********obj_path*****************************")
                print(info.path, read_obj(info.path, decompose=False))
...
def approximate_as_prism(body, body_pose=unit_pose(), **kwargs):
    ...
    print("***************verticles*************************************")
    print(vertices_from_rigid(body, **kwargs))
    print(vertices)
...
def get_difference_fn(body, joints):
    ...
    def fn(q2, q1):
        print("***************get_difference_fn*********************")
        print("circular", circular_joints)
...
def get_extend_fn(body, joints, resolutions=None, norm=2):
    # norm = 1, 2, INF
    # da xiang jiao ,yi gen da xiang jiao, piao a piao, yao a yao
    # ban shan yao, you yi zuo shan shen miao , hahahahahahaha, tmd
...
def inverse_kinematics(robot, link, target_pose, max_iterations=200, max_time=INF, custom_limits={}, **kwargs):
    ...
    # limit_space = ([-2*math.pi, -2*math.pi, -math.pi, -2*math.pi, -2*math.pi, -2*math.pi],
    #                [2*math.pi, 2*math.pi, math.pi, 2*math.pi, 2*math.pi, 2*math.pi],
    #                 [4*math.pi, 4*math.pi, 2*math.pi, 4*math.pi, 4*math.pi, 4*math.pi],
