...
from examples.pybullet.utils.pybullet_tools.kuka2_primitives import BodyPose, BodyConf, Command, get_grasp_gen, \
...
def pddlstream_from_problem(robot, goal, movable=[], fixed=[], init = [], teleport=False, grasp_name='top'):
    ...
    # goal = ('and',
    #         ('AtConf', conf),
    #         ('Cleared_slot', body2, body, fixed[2])
    #         # ('Occupied', fixed[2])      
    # )
    
    # goal = ('Pick_gaizi', body_0, fixed[2])
    # if OCCUPIED_FLAG :
    #     
    #     goal = ('and',
    #         ('AtConf', conf),
...
def get_goal(robot,movable=[],):
    ...
    goal = ('and',
            ('AtConf', conf),
            ('Pick_gaizi', body_0, fixed[2]),
            # ('Pick_gaizi_to_target_region', movable[0], fixed[2]),
            # ('Complete_task', body_0, body3 ,fixed[2])
            # ('Remove_block', body, body2)
            # (not ('Occupied', fixed[2]))
            # ('On', body_0, fixed[2])
            # ('On', body3, fixed[2])
            )
    goal2 = ('and',
...
def main():
    ...
    problem = pddlstream_from_problem(robot, goal=goal2, movable=movable, fixed=fixed, init=init, teleport=args.teleport)
    ...
    if args.simulate:
        ...
        # command.execute(time_step=0.01)
